          ...
          */
            ...
            if (empty($member)) {
                ...
                if ($registerIntegral && $registerIntegral > 0) {
                    ...
                    m('member')->setAddSpeed($userId, $sysset['shop']['to_money_basic_speed'], 1, '', '用户注册获取基础加速值');
                }
                //直推下载注册奖励
                if (!empty($_GPC['mid'])) {
                    $inviterLevel = pdo_getcolumn("ewei_shop_member", array('id' => $_GPC['mid']), 'level');
                    $inviteSpeed = pdo_getcolumn("ewei_shop_member_level", array('id' => $inviterLevel), 'invite_speed');
                  	$inviteSpeed = $inviteSpeed ?: 3;
                    m('member')->setAddSpeed($_GPC['mid'], $inviteSpeed, 2, $userId, '邀请用户' . $userId . '下载注册获取');
                }
