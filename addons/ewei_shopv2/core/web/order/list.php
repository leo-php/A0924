...
class List_EweiShopV2Page extends WebPage
{
	protected function orderData($status, $st)
	{
		...
		//增加等级搜索
		if(!empty($_GPC['levellike'])){
			if($_GPC['levellike']!='level'){
				if($_GPC['levellike']=='vip'){
					$_GPC['levellike']=0;
				}
				$condition .= ' and m.level='.$_GPC['levellike'];
			}
		}
...
';
		...
		//增加等级搜索
		$member_level=pdo_fetchall(' select id,levelname from '.tablename('ewei_shop_member_level'));
