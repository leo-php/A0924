...
class Order_EweiShopV2Model
{
    ...
    public function payVirtualSend($orderid = 0, $ispeerpay = false)
    {
        global $_W;
        global $_GPC;
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy,address from ' . tablename('ewei_shop_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
    ...
    public function getOrderDispatchPrice($goods, $member, $address, $saleset = false, $merch_array, $t, $loop = 0)
    {
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($g['issendfree'])) {
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0) {
                    ...
                    //增加普通会员包邮，代理不包邮，这是满多少件包邮
                    $dispatch_data['good_number']=0;
                    ...
                    if($g['free_shipping']==1 ) {
                        if(empty($agent_level['level']) || $agent_level['level'] == $level['id']){
                            $good_ednum=$g['ednum'] ;
                           // $good_number=1;
        ...
        if (!empty($dispatch_array)) {
            ...
            foreach ($dispatch_array as $k => $v) {
                ...
                if (!empty($address)) {
                    //计算邮费，按地区判断，增加普通会员包邮@free_shipping
                    $dispatch_data['good_ednum']=$good_ednum;
                    //$dispatch_data['good_number']=$good_number;
                    $dispatch_data['free_shipping']=$free_shipping;
