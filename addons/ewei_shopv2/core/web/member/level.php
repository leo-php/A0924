    ...
    protected function post()
    {
        ...
        if ($id == 'default') {
            ...
            if (!empty($level)) {
                ...
                $inviteBuyGoodsids = iunserializer($level['invite_buy_goodsids']);
                if (!empty($inviteBuyGoodsids)) {
                    $inviteBuyGoods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $inviteBuyGoodsids) . ')', array(':uniacid' => $_W['uniacid']));
                }

                $inviteTwoBuyGoodsids = iunserializer($level['invite_two_buy_goodsids']);
                if (!empty($inviteTwoBuyGoodsids)) {
                    $inviteTwoBuyGoods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $inviteTwoBuyGoodsids) . ')', array(':uniacid' => $_W['uniacid']));
                }
            }
        }
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'enabled' => $enabled,
                'same_level_invite' => trim($_GPC['same_level_invite']),
                'invite_speed' => trim($_GPC['invite_speed']),
                'invite_buy_speed' => trim($_GPC['invite_buy_speed']),
                'invite_buy_goodsids' =>serialize($_GPC['invite_buy_goodsids']),
                'invite_buy_point' => trim($_GPC['invite_buy_point']),
                'invite_two_buy_speed' => trim($_GPC['invite_two_buy_speed']),
                'invite_two_buy_goodsids' =>serialize($_GPC['invite_two_buy_goodsids']),
                'invite_two_buy_point' => trim($_GPC['invite_two_buy_point']),
                'area_type' => trim($_GPC['area_type']),
                'area_buy_point' => trim($_GPC['area_buy_point']),
                'stock_money' => trim($_GPC['stock_money']),
                'invite_buy_max_speed' => trim($_GPC['invite_buy_max_speed']),
                'invite_two_buy_max_speed' => trim($_GPC['invite_two_buy_max_speed']),
                'invite_buy_speed_unit' => trim($_GPC['invite_buy_speed_unit']),
                'invite_two_buy_speed_unit' => trim($_GPC['invite_two_buy_speed_unit']),
                'double_credit' => $_GPC['double_credit'],
                'rebuy_double_credit' => $_GPC['rebuy_double_credit'],
                'rebuy_speed' => $_GPC['rebuy_speed']
            );
        ...
        //上一等级名称
        $upLevel = pdo_fetch("select levelname from ".tablename("ewei_shop_member_level")." where level<(select level from ims_ewei_shop_member_level where id=:id) order by `level` desc limit 1",array(':id'=>$id));
        include $this->template();
    }
