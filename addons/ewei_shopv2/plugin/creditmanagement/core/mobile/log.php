...
class Log_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
        ...
        $list = pdo_fetchall('select * from ' . tablename('ewei_shop_creditmanagement_log') . ' where 1 ' . $condition . ' order by id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_creditmanagement_log') . ' where 1 ' . $condition, $params);
