...
class Bind_EweiShopV2Model
{
    ...
    public function update($mid = 0, $arr = array())
    {
        ...
        pdo_update('ewei_shop_member', $arr, array(
            'id' => $mid,
            'uniacid' => $_W['uniacid']
        ));
    }
    ...
    public function merge($a = array(), $b = array())
    {
        ...
        $comparelevel = m('member')->compareLevel(array(
            $a['level'],
            $b['level']
        ));
        ...
        if (!empty($a['isagent']) && empty($b['isagent'])) {
            ...
            if (!empty($b['isagent']) && empty($a['isagent'])) {
                ...
                if (!empty($b['isagent']) && !empty($a['isagent'])) {
                    $compare = p('commission')->compareLevel(array(
                        $a['agentlevel'],
                        $b['agentlevel']
                    ));
                    $isagent = 1;
        ...
        pdo_update('ewei_shop_commission_apply', array('mid' => $b['id']), array(
            'uniacid' => $_W['uniacid'],
            'mid' => $a['id']
        ));
        ...
        //加速度合并
        if (0 < $a['exchange_speed']) {
            m('member')->setAddSpeed($b['id'], $a['exchange_speed'], 9, '', '用户合并数据');
        }

        //修改下级和记录前用户id
        $allInviterIds = pdo_fetchall("select id from " . tablename("ewei_shop_member") . " where inviter={$a['id']}");
        foreach ($allInviterIds as $k => $v) {
            pdo_update("ewei_shop_member", array('inviter' => $b['id']), array('id' => $v['id']));
        }
        $updateData = array(
            'old_id' => $a['id'],
            'old_level' => $a['level'],
            'old_agent_area' => $a['agent_area'],
            'old_agent_city' => $a['agent_city'],
            'old_agent_province' => $a['agent_provice']
        );
        pdo_update("ewei_shop_member", $updateData, array('id' => $b['id']));
        ...
        foreach ($tables as $k => $v) {
            ...
            if (pdo_fieldexists($tablename, 'openid') && pdo_fieldexists($tablename, 'uniacid')) {
                pdo_update($tablename, array('openid' => $b['openid']), array(
                    'uniacid' => $_W['uniacid'],
                    'openid' => $a['openid']
                ));
            }
            ...
            if (pdo_fieldexists($tablename, 'mid') && pdo_fieldexists($tablename, 'uniacid')) {
                pdo_update($tablename, array('mid' => $b['id']), array(
                    'uniacid' => $_W['uniacid'],
                    'mid' => $a['id']
                ));
            }
