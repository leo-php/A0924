...
if ($do == 'display') {
	...
	if (!empty($tag)) {
		$select_sql = "SELECT %s FROM " .tablename('mc_mapping_fans')." AS f LEFT JOIN ".tablename('mc_fans_tag_mapping')." AS m ON m.`fanid` = f.`fanid` " . "LEFT JOIN " . tablename('mc_fans_tag') . " AS t ON t.`fanid` = f.`fanid` " . $condition ." %s";
	} else {
		$select_sql = "SELECT %s FROM " .tablename('mc_mapping_fans')." AS f LEFT JOIN " . tablename('mc_fans_tag') . " as t ON t.`fanid` = f.`fanid` " . $condition ." %s";
	}
	$fans_list_sql = sprintf($select_sql, "f.fanid, f.acid, f.uniacid, f.uid, f.openid, f.nickname as nickname, f.groupid, f.follow, f.followtime, f.unfollowtime, t.headimgurl", " GROUP BY f.`fanid` ORDER BY f.`fanid` DESC LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize);
