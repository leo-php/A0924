...
//                高级
    ...
    public function test(){
        ...
        //默认等级成为店长
        $member2=pdo_fetchall(' select id from '.tablename('ewei_shop_member').' where uniacid='.$_W['uniacid'] .' and level=0');
        $level=pdo_fetch(' select id,level from '.tablename('ewei_shop_member_level').' where uniacid='.$_W['uniacid'] .' and level=1 ');
        foreach($member2 as $value){
            pdo_update('ewei_shop_member', array('level' => $level['id']), array('uniacid' => $_W['uniacid'], 'id' => $value['id']));
        }

        //老会员成为店长
        $level1=pdo_fetch(' select id,level from '.tablename('ewei_shop_member_level').' where uniacid='.$_W['uniacid'] .' and level=0 ');
        $member3=pdo_fetchall(' select id from '.tablename('ewei_shop_member').' where uniacid='.$_W['uniacid'] .' and level='.$level1['id']);
        ...
        //转移邀请人id至分销商上级ID
//        $member4=pdo_fetchall(' select id,agentid,inviter,isagent,status from '.tablename('ewei_shop_member').' where uniacid='.$_W['uniacid'] .' and inviter!=0 ');
//        //dddd($member4);
