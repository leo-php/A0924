        ...
        if (!$packageid) {
            ...
            if (empty($id)) {
                ...
                if (!empty($id) && !empty($iswholesale)) {
                    ...
                    $sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,isverify,isforceverifystore,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,diyfields,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale,bargain,free_shipping' . $threensql . $ishotelsql . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
                    $data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
...
//        }
        ...
        if($agent_level['level']!=0 && $agent_level['level'] != $level['id'] ){
            ...
            if($data['free_shipping']!=0 && !in_array($address['city'],$data_edareas)){
                if(0<$data['ednum']){
                    $return_array['price'] = $dispatch_price + $seckill_dispatchprice;
                }else{
                    $return_array['price'] = 0;
